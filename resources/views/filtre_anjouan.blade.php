...
<body> 
    <div class="container"> <br> <br>
        <h1 class="text-center">Filtrage anjouan</h1> <br> <br>
        <form action="{{route('photo_anjouan')}}" method="post">
            @csrf
            <div class="form-group">
                <select class="form-select" id="faculte" name="departement" aria-label="Default select example" required>
                    <option selected value="">Departement</option>
                    @foreach ( $facultes as $faculte )
                    <option value="{{$faculte->code_depart}}">{{$faculte->design_depart}}</option>
                    @endforeach
                </select>
            </div> <br> 
            <div class="form-group">
